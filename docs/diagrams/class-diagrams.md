...
This document specifies the UML Class Diagrams for the **Class Manager** system based directly on the active implementation across `src/types/`, `src/services/`, `src/lib/store.ts`, and `src/lib/auth.ts`.
...
This diagram captures the structural relationships between the school domain entities defined in `src/types/index.ts`.
