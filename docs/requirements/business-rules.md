...
* **Enforced By:**
  - `frontend/src/lib/constants.ts` (`CLASS_CONSTANTS.DESK_COUNT = 20`, `MAX_SEATS = 40`, `COLS = 4`, `ROWS = 5`)
...
* **Enforced By:**
  - `frontend/src/services/student.service.ts` (`createStudent` & `importStudents`)
  - `backend/migrations/001_initial_schema.sql` (`check_max_students()` trigger)
  - `frontend/src/lib/validations/forms.ts` (`max_students: z.coerce.number().min(1).max(40)`)
...
* **Enforced By:**
  - `frontend/src/services/seating.service.ts` (`assignSeat`)
  - `frontend/src/lib/store.ts` (`assignSeat`)
...
* **Enforced By:**
  - `frontend/src/services/teacher.service.ts` (`checkGradeLimit`)
  - `frontend/tests/teacher-validation.test.ts`
...
* **Enforced By:**
  - `frontend/src/lib/store.ts` (`assignHomeroomTeacher`)
...
* **Enforced By:**
  - `frontend/src/lib/store.ts` (`assignSubjectTeacher`)
...
* **Enforced By:**
  - `frontend/src/lib/auth.ts` (`login`, `getCurrentUser`)
  - `frontend/src/services/auth-guard.ts` (all methods check `user.status !== 'disabled'`)
...
* **Enforced By:**
  - `frontend/src/lib/constants.ts` (`GRADE_SHIFTS`, `getClassAllowedPeriods`)
  - `frontend/src/services/timetable.service.ts` (`validateTimetableEntry`)
...
* **Enforced By:**
  - `frontend/src/lib/constants.ts` (`getClassHomeroomSlot`)
  - `frontend/src/services/timetable.service.ts` (`validateTimetableEntry`)
...
* **Enforced By:**
  - `frontend/src/lib/constants.ts` (`DAY_ALLOWED_PERIODS`)
  - `frontend/src/services/timetable.service.ts` (`validateTimetableEntry`)
...
* **Enforced By:**
  - `frontend/src/services/timetable.service.ts` (`checkTeacherConflict`)
...
* **Enforced By:**
  - `frontend/src/services/timetable.service.ts` (`checkClassConflict`)
...
* **Enforced By:**
  - `frontend/src/services/auth-guard.ts` (`canManageTimetable`)
  - `frontend/src/services/timetable.service.ts` (`saveEntry`, `deleteEntry`, `copyTimetable`)
...
* **Enforced By:**
  - `frontend/src/services/auth-guard.ts` (`canManageAttendance`)
  - `frontend/src/services/attendance.service.ts` (`saveAttendanceBatch`)
